...
function createPlan(args) {
    ...
    //     console.log("Step",i,":", newPlan.steps[i]);
    // }

    newPlan.steps.forEach(function (step, i) {
...
function executePlan(currentPlan) {
    if (!currentPlan) {
        return {
            error: "No plan has been created. Call createPlan first"
        }
    }
    console.log("Executing new plan:", currentPlan.goal);
    return {
        message: "Plan execution has started.", plan: currentPlan
    }
}
...
// call this when the agent performs one tool call (aka one dispatch request)
// assuming that one tool call means doing one step in the plan
...
async function agentLoop(messages, maxIterations = 10) {
    ...
    while (iterations < maxIterations) {
        ...
        // thoughtSummary will be an array consisting of elements from modelContent.parts
        // that returns true when passed to the filter function
        const thoughtSummary = modelContent?.parts?.filter(function(part){
            // see if part.thought is true and whether part.text is string
            return part.thought === true && typeof part.text === "string"
        }).map(part => part.text).join("") ?? "";

        // The long form:
        // const thoughtSummary = [];
        // for (let part of modelContent.parts) {
        //     if (part.thought && typeof part.text === "string") {
        //         thoughtSummary.push(part.text);
        //     }
        // }
        // const thoughtSummaryString = thoughtSummary.join("");
        // // thoguhtSummary will be an array of strings: ["abc", "def"]
        // // to converted into a string, we use .join("");
        // // ["abc", "def"].join("") => "abcdef"
